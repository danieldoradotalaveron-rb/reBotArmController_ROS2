...
`/rebotarm/follow_joint_trajectory` 会接收上层直接给出的关节数组；ROS action 只做基础消息格式
和关节名顺序检查，并把最后一个 point 交给 SDK `ArmEndPos` 作为目标，不再做 URDF joint limit
二次校验。
...
说明：切回 `pos_vel` 控制并调用 SDK `ArmEndPos.safe_home()`，让机械臂以安全速度回零。调用前会先停止重力补偿。
...
说明：标准关节轨迹 action 兼容入口。当前实现是 SDK 薄封装：只读取最后一个
trajectory point 的 `positions`，将其作为 SDK `ArmEndPos` 的关节目标；实际发送由
SDK 的 `pos_vel` 控制循环完成。action 存活期间会把当前 `/joint_states` 对应的
positions / velocities 填入 `FollowJointTrajectory.Feedback.actual`。

约束：

- `joint_names` 必须与底层 `RobotArm.joint_names` 顺序完全一致。
- 最后一个 trajectory point 必须包含完整 `positions`。
- 当前不会在 ROS 层回放多段轨迹，也不会重复实现 SDK / URDF 的合法性检查。
- 最后一个 point 的 `time_from_start` 只作为 action feedback 的持续时间窗口。
...
- `/follow_joint_trajectory` 当前只使用最后一个 point 作为 SDK 关节目标；多 point 不会在 ROS 层逐点回放。
- `/move_to_pose` 更适合应用层“到达某个末端位姿”的常规使用。
- 重力补偿必须在 controller 内部运行；不要在外部 ROS 节点用 `/joint_states` + raw command 重写高频闭环。
- `GripperControl` demo 退出时会先 close，再 disable；如果使用 API 自己写应用，也建议采用同样顺序。
- 多机械臂场景中，`arm_namespace` 只解决 ROS graph 命名冲突；TF frame 仍需额外规划 frame 前缀或 URDF 命名。
- 修改 `hardware_manager.py`、`ros_services.py` 或 `ros_actions.py` 后，需要重启 `bringup.launch.py` 才会加载新 controller 逻辑。
